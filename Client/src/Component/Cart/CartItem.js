...
const CartItem = ({ cart }) => {
  ...
  useEffect(() => {
    setTotalPriceOneProduct(parseInt(cart.price) * cart.quantity);
  }, [cart.quantity, cart_manager]);
  //  -----------------------------------------------------------------------------------------------------------------------
  ...
  useEffect(() => {
    cartUser &&
      ...
      cartUser.length !== 0 &&
      dispatch({ type: UPDATE_CART, cart: cartUser });
  }, [cartUser]);
  ...
  return (
    cart && (
      <div className="container">
        <div className="row">
          <div className="col-2 ">
            <img className="img-thumbnail " src={`${cart.image}`} />
          </div>
          <div className="col-2 ">
            <p className="$font-weight-light">{cart.name}</p>
          </div>
          <div className="col-2 ">
            <p className="">{separateNumber(cart.price)}</p>
          </div>

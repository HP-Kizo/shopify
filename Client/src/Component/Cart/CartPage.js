...
import { ADD_CART, UPDATE_CART } from "../../store/context";
...
function CartPage() {
  ...
  const { data, isLoading, status, error, reFetch } =
    useFetch("/carts/getCarts");
  useEffect(() => {
    if (status === 200) {
      dispatch({ type: UPDATE_CART, cart: data.products });
    }
  }, [data]);
  //  -----------------------------------------------------------------------------------------------------------------------
  ...
  return (
    <Fragment>
      ...
      <div className=" container   bg-light text-center">
        ...
        {!isLoading && status === 200 && data.products.length > 0 && (
